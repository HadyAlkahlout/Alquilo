import 'package:flutter/material.dart';

class ColorManager {
  static const Color primaryColor = Color(0xFF2970FF);
  static const Color primaryLightColor = Color(0x302970FF);
  static const Color secondaryColor = Color(0xFF6EA8FF);
  static const Color backgroundLight = Color(0xFFF8FAFC);
  static const Color backgroundDark = Color(0xFF0F172A);

  static const Color lightGrey = Color(0xFFCDD5DF);
  static const Color darkGrey = Color(0xFF364152);

  static const textLight = Color(0xFF4B5565);
  static const textDark = Colors.white;
}